...
public class C3_MultiArrays_StreamOrnekleri {
    ...
    public static void doubleYaz (String [][] str){
         Arrays.stream(str).flatMap(t-> Arrays.stream(t)).filter(t-> t.toLowerCase().startsWith("e")).
    ...
    public static List<String> eLeriListeYaz (String [][] str){
        return Arrays.stream(str).flatMap(t-> Arrays.stream(t)).filter(t->t.toLowerCase().
                startsWith("e")).collect(Collectors.toList());
    }
    //S4 : k ile bitenlerin sonuna '*' ekleyelim
    public static void kBitenleriYildiz (String [][] str) {
        Arrays.stream(str).flatMap(t -> Arrays.stream(t)).filter(t -> t.endsWith("k")).map(t -> t + "*").
